  ...
  for (i =0; i <  gnum_full_fluents_initial; i ++)
    {
      ...
      ff = gfull_fluents_initial[i].fluent.function;
      printf("\n %s", gfunctions[ff]);
      if (strcmp(gfunctions[ff],"REWARD_STUDENT") == SAME) 
	{
	 print_Fluent( &(gfull_fluents_initial[i].fluent));

      for ( j = 0; j < gf_arity[ff]; j++ ) {
        printf("  args[%d]=%f", j, gfull_fluents_initial[i].fluent.args[j]);
      }

	 printf(": %f", gfull_fluents_initial[i].value);
          reward = gfull_fluents_initial[i].value;

	}
    }

       if (reward != 0)
	 printf( "\n\nTotal reward: %7.2f", reward);
